...
func TestBuildAPIKeyClientsCounts(t *testing.T) {
	cfg := &config.Config{
		...
		ClaudeKey: []config.ClaudeKey{{APIKeyEntries: []string{"c1", "c2"}}, {APIKey: "c3"}},
		CodexKey:  []config.CodexKey{{APIKey: "x1"}, {APIKeyEntries: []string{"x2", "x3"}}},
	...
	if gemini != 2 || vertex != 1 || claude != 3 || codex != 3 || compat != 2 {
